...
export function queryPaginationValidator (req: Request, res: Response, next: NextFunction): void {
  const paginationSchema: Zod.Schema = z.object({
    skip: z.string().optional()
      .transform((val) => parseInt(val ?? DEFAULT_SKIP_STRING_VALUE))
      .refine(val => !isNaN(val) && val >= 0, 'skip must be a number >= 0'),
    limit: z.string().optional()
      .transform((val) => parseInt(val ?? DEFAULT_LIMIT_STRING_VALUE))
