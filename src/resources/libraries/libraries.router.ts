...
librariesRouter.get('/', queryPaginationValidator, tryCatch(async (req) => await librariesService.list(req.userId ?? '', req.role ?? Role.Regular, parseSkipLimitQP(req))))
...
// Libraries-books
librariesRouter.get('/:id/books', queryPaginationValidator, tryCatch(async (req) => await librariesBooksService.list(req.params.id, req.userId ?? '', parseSkipLimitQP(req))))
