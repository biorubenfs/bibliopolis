...
async function fetchISBN10(isbn13: string): Promise<string | null> {
  try {
    ...
    if (data.isbn_10 && data.isbn_10.length > 0) {
      console.log(`  ✓ ISBN-10 encontrado: ${data.isbn_10[0]}`)
...
async function enrichBooks() {
  ...
  const booksToEnrich = books.filter(book => book.isbn_10 === null)
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    if (book.isbn_10 !== null) {
      console.log(`[${i + 1}/${books.length}] "${book.title}" - Ya tiene ISBN-10: ${book.isbn_10}`)
      continue
    }
    
    console.log(`[${i + 1}/${books.length}] "${book.title}" (ISBN-13: ${book.isbn_13})`)
    
    const isbn10 = await fetchISBN10(book.isbn_13)
    
    if (isbn10) {
      books[i].isbn_10 = isbn10
      enrichedCount++
    } else if (isbn10 === null) {
      notFoundCount++
    } else {
